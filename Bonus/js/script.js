...
createApp({
    data() {
        return {
            ...
            //contiene il testo della search bar
            searchText: ''
        }
    },
    ...
    methods: {
        ...
        deleteMessage(index) {
            this.contacts[this.activeImage].messages.splice(index, 1);
        },

        //funzione per ottenere la data odierna
        newDate(){
            let actualDate = luxon.DateTime;
            let today = actualDate.now().toLocaleString(actualDate.DATETIME_SHORT_WITH_SECONDS)
            return today
         }
    }
}).mount('#app');